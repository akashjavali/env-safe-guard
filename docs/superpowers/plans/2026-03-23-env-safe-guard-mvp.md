# env-safe-guard MVP Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `env-safe-guard` — a TypeScript npm package that validates environment variables, provides full type inference, and acts as a security layer preventing secrets from leaking into logs, AI tools, and git history.
...
3. Runs `env-safe-guard check` if `env-schema.json` exists
...
```json
{
  ...
  "types": "./dist/index.d.ts",
  "bin": {
    "env-safe-guard": "./dist/cli/index.js"
  },
...
function warnIfSecretLooking(schema: EnvSchema, parsed: Record<string, unknown>): void {
  ...
  for (const [key, value] of Object.entries(parsed)) {
    ...
    if (typeof value === 'string' && looksLikeSecret(value)) {
      console.warn(
        `[env-safe-guard] ⚠️  ${key} looks like a secret. Consider marking it secret: true in the schema.`,
      );
    }
  }
}
...
# Auto-installed by: npx env-safe-guard install-hook
...
if [ -f "env-schema.json" ]; then
  if command -v npx >/dev/null 2>&1; then
    npx env-safe-guard check --schema env-schema.json
    if [ $? -ne 0 ]; then
